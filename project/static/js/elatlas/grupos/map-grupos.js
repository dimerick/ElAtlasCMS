...
layers.push(CartoDB_DarkMatter);
        ...
        // L.control.iconL
        var map = L.map('map', {
            center: [5.659718554577286, -73.11401367187501],
            zoom: 6,
            maxZoom: 18,
            minZoom: 3,
        ...
        function getSitios(){
            jQuery.ajax({
                ...
                success: function (data, textStatus, jqXHR){
                    ...
                    dataJson = JSON.stringify(grupos);
...
//                    }).addTo(map);
                    ...
                    var gruposMap = L.Proj.geoJson(dataJson,{
                        onEachFeature: eachGrupo
                    });
...
//
        ...
        function eachGrupo(feature, layer){
            numGroups++;
            if(layer != null){
...
//                console.log(layer);
                ...
                var categories = [];
                if(feature.geometry.properties.categorias){
                    for(var i=0;i<feature.geometry.properties.categorias.length;i++){
                        categories.push(feature.geometry.properties.categorias[i].nombre);
                    }
                }

                var image = '';
                if(feature.geometry.properties.foto){
                    var urlImg = feature.geometry.properties.foto;
                    if(urlImg != 'photo-profile.jpg'){
                        image = '<img src="/static/images/elatlas-grupos/'+urlImg+'">';
                ...
                var content = '<div class="content-info-marker">' + title +
                    ...
                    '<li> <i class="fa fa-caret-right"></i> <span><b>Ciudad: </b>'+' '+feature.geometry.properties.ciudad+'</span></li>' +
                    '<li> <i class="fa fa-caret-right"></i> <span><b>No. Integrantes: </b>'+' '+feature.geometry.properties.num_int+'</span></li>' +
                    '<li> <i class="fa fa-caret-right"></i> <span><b>Categorias: </b>'+' '+categories.join(", ")+'</span></li>' +
                    '<li> <i class="fa fa-caret-right"></i> <span><b>Descripción: </b>'+' '+feature.geometry.properties.descripcion+'</span></li>' +
                    '</ul>' + 
                    image
